...
if [ -e ~/.aider.conf.yml ]; then
  echo "Aider config found, mounting it into the container..."
  DEVCONTAINER_OPTIONS="$DEVCONTAINER_OPTIONS --mount type=bind,source=$(realpath ~/.aider.conf.yml),target=/tmp/aider.conf.yml"
fi
...
devcontainer exec \
  --workspace-folder "$REPO_PATH" \
  /bin/bash -c "git config --global --add safe.directory /workspaces/$REPO_NAME"
